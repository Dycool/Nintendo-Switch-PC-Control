...
where cl >nul 2>&1
if %ERRORLEVEL% == 0 (
    cl /std:c++17 /O2 /EHsc /W3 ns-gamepad.cpp /link ws2_32.lib xinput.lib winmm.lib /out:gamepad.exe
    goto :gui
)

where g++ >nul 2>&1
if %ERRORLEVEL% == 0 (
    g++ -std=c++17 -O2 -Wall ns-gamepad.cpp -o gamepad.exe -static -lws2_32 -lxinput -lwinmm
    goto :gui
)

if exist "C:\msys64\ucrt64\bin\g++.exe" (
    C:\msys64\ucrt64\bin\g++.exe -std=c++17 -O2 -Wall ns-gamepad.cpp -o gamepad.exe -static -lws2_32 -lxinput -lwinmm
)
