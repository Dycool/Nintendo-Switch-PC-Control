#!/bin/bash
# setup_probe.sh — Standalone USB gadget probe management
# Usage:
...
setup() {
    echo "[probe] Setting up USB gadget probe..."
    ...
    echo 0x057e > "$PROBE_DIR/idVendor"
    echo 0x2009 > "$PROBE_DIR/idProduct"
    ...
    echo "NS PC Control"      > "$PROBE_DIR/strings/0x409/manufacturer"
    echo "USB Gamepad Probe"  > "$PROBE_DIR/strings/0x409/product"
    ...
    echo "[probe] USB gadget probe on /dev/hidg0 (bound to $UDC)"
}
...
case "$cmd" in
    ...
    *)
        ...
        echo ""
        echo "  setup      Create and bind USB gadget probe (replaces ns_ctrl)"
