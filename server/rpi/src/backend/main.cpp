...
static MultiReport g_multi_report{};
...
// ── Thread 1: HORI (Single Player) ──
static void hori_thread(int hz) {
    const auto tick = us(1'000'000 / hz);
    const int af_period = std::max(1, hz / (AUTOFIRE_HZ * 2));
    int af_tick = 0; uint16_t af_state = 0;
    int fd = -1; bool was_connected = false;
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        HIDReport prev{}; prev.buttons = 0xFFFF; 
        ...
        while (g_running.load(std::memory_order_relaxed)) {
            ...
            HIDReport r; uint16_t af_mask;
            {
                std::lock_guard<std::mutex> lk(g_mtx);
                if (silent) g_hori_report.reset();
                r = g_hori_report; af_mask = g_autofire_mask;
            }
...
// ── Thread 2: Multi-HORI Hub (Multiplayer) ──
static void multi_thread(int hz) {
    const auto tick = us(1'000'000 / hz);
    int fds[4] = {-1, -1, -1, -1};
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (g_verbose || !was_connected) std::puts("[backend] 4x /dev/hidg* opened (MULTI Mode)");
        was_connected = true;
        ...
        while (g_running.load(std::memory_order_relaxed)) {
            ...
            {
                std::lock_guard<std::mutex> lk(g_mtx);
                if (silent) g_multi_report.reset();
                r = g_multi_report;
            }
            ...
            if (!ok) {
                if (!error_shown) { std::puts("[backend] Disconnected from Switch..."); error_shown = true; }
...
static bool rate_allow(uint32_t ip) {
    uint64_t now = now_us(); uint32_t idx = ip % RATE_TABLE;
    RateSlot &s = g_rate_table[idx];
    if (s.ip != ip || now - s.window_start > RATE_WINDOW_US) { s.ip = ip; s.count = 1; s.window_start = now; return true; }
    return ++s.count <= RATE_MAX_PKT;
}

int main(int argc, char** argv) {
    uint16_t port = DEFAULT_PORT; std::string bind_addr = "0.0.0.0";
    ...
    sockaddr_in addr{}; addr.sin_family = AF_INET; addr.sin_port = htons(port); addr.sin_addr.s_addr = inet_addr(bind_addr.c_str());
    bind(sock, (sockaddr*)&addr, sizeof(addr));

    std::printf("[backend] Mode: %s | UDP %s:%u\n", g_multiplayer ? "MULTI (4x HORI)" : "SINGLE (HORI)", bind_addr.c_str(), port);

    std::thread wt([&]() {
        if (g_multiplayer) multi_thread(WRITER_HZ);
        else               hori_thread(WRITER_HZ);
    });
    ...
    Packet pkt{}; uint32_t expected_seq = 0; bool first_pkt = true; epoll_event evs[4];
    ...
    while (g_running.load(std::memory_order_relaxed)) {
        ...
        if (!first_pkt && seq_diff < 0 && !is_reset && seq_diff > -100) continue;
        first_pkt = false; expected_seq = pkt.seq + 1;

        {
            std::lock_guard<std::mutex> lk(g_mtx);
            if (is_reset) {
                if (g_multiplayer) g_multi_report.reset();
                else g_hori_report.reset();
                g_autofire_mask = 0;
            } else {
                if (g_multiplayer) g_multi_report = pkt.payload.multi;
                else { g_hori_report = pkt.payload.single; g_autofire_mask = (pkt.flags & FLAG_AUTOFIRE) ? pkt.autofire_mask : 0; }
            }
        }
